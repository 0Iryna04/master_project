...
async function makePrediction(pair = 'BTC_USD') {
    try {
        ...
        await trainModel(model, normalizedData);

        const predictedPrice = await predictNextPrice(model, normalizedData);
        console.log(`Predicted price for ${pair}: ${predictedPrice}`);
...
let timerBut1, timerBut2, timerBut3, timerStop = false, 
    ...
    document.getElementById('predictBtn').addEventListener('click', async () => {
        const predictedPrice = await makePrediction('BTC_USD');
    ...
    document.getElementById("predictButton").addEventListener("click", async function () {
        ...
        document.getElementById("predictionResult").textContent = `Прогнозована ціна: ${result.prediction}`;
    });
...
async function executeTrade(decision) {
    ...
    // Логіка для купівлі/продажу
    if (decision === 'buy') {
        // Виконати купівлю
        console.log("Виконання покупки на біржі");
        // ваш код для купівлі
    } else if (decision === 'sell') {
        // Виконати продаж
        console.log("Виконання продажу на біржі");
        // ваш код для продажу
    }
}
